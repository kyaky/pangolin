//! Prometheus metrics endpoint for a running `pgn connect` session.
//!
...
pub fn render_scrape(state: &MetricsState, now: Instant) -> String {
    ...
    out.push_str("# HELP pangolin_session_info Static metadata about the running pgn session.\n");
    out.push_str("# TYPE pangolin_session_info gauge\n");
    out.push_str("pangolin_session_info{");
    write_labels(
        &mut out,
        &[
            ("instance", instance),
    ...
    out.push_str("# HELP pangolin_session_state Current session state (one series per state).\n");
    out.push_str("# TYPE pangolin_session_state gauge\n");
    for (label, this) in [
        ...
        out.push_str("pangolin_session_state{");
    ...
    out.push_str(
        "# HELP pangolin_session_started_at_unix Unix timestamp when the session started.\n",
    );
    out.push_str("# TYPE pangolin_session_started_at_unix gauge\n");
    out.push_str("pangolin_session_started_at_unix{");
    write_labels(&mut out, &[("instance", instance)]);
    out.push_str(&format!("}} {}\n", base.started_at_unix));

    // uptime_seconds: derived from a stored Instant.
    out.push_str("# HELP pangolin_session_uptime_seconds Seconds since the session started.\n");
    out.push_str("# TYPE pangolin_session_uptime_seconds gauge\n");
    out.push_str("pangolin_session_uptime_seconds{");
    write_labels(&mut out, &[("instance", instance)]);
    out.push_str(&format!("}} {uptime}\n"));

    // routes: count of split-tunnel routes installed.
    out.push_str("# HELP pangolin_session_routes Number of split-tunnel routes installed.\n");
    out.push_str("# TYPE pangolin_session_routes gauge\n");
    out.push_str("pangolin_session_routes{");
    write_labels(&mut out, &[("instance", instance)]);
    out.push_str(&format!("}} {}\n", base.routes.len()));
    ...
    out.push_str(
        "# HELP pangolin_reconnect_attempts_total Reconnect attempts driven by the app-level state machine.\n",
    );
    out.push_str("# TYPE pangolin_reconnect_attempts_total counter\n");
    out.push_str("pangolin_reconnect_attempts_total{");
    write_labels(&mut out, &[("instance", instance)]);
    out.push_str(&format!("}} {reconnects}\n"));

    out.push_str(
        "# HELP pangolin_tunnel_restarts_total Successful tunnel re-establishments (post-backoff).\n",
    );
    out.push_str("# TYPE pangolin_tunnel_restarts_total counter\n");
    out.push_str("pangolin_tunnel_restarts_total{");
    write_labels(&mut out, &[("instance", instance)]);
...
mod tests {
    ...
    fn render_includes_info_block_with_labels() {
        ...
        assert!(out.contains("pangolin_session_info{"));
    ...
    fn render_emits_one_state_series_per_value() {
        ...
        assert!(out.contains(r#"pangolin_session_state{instance="work",state="connected"} 1"#));
        assert!(out.contains(r#"pangolin_session_state{instance="work",state="connecting"} 0"#));
        assert!(out.contains(r#"pangolin_session_state{instance="work",state="reconnecting"} 0"#));
    }
    ...
    fn render_uptime_is_deterministic_from_instant() {
        ...
        assert!(out.contains("pangolin_session_uptime_seconds{instance=\"work\"} 0\n"));
    ...
    fn render_counters_reflect_atomic_state() {
        ...
        let out = render_scrape(&s, Instant::now());
        assert!(out.contains("pangolin_reconnect_attempts_total{instance=\"work\"} 7\n"));
        assert!(out.contains("pangolin_tunnel_restarts_total{instance=\"work\"} 3\n"));
    }
    ...
    fn render_routes_count_matches_vec_len() {
        ...
        assert!(out.contains("pangolin_session_routes{instance=\"work\"} 0\n"));
        let mut b = base("work");
        b.routes = vec!["a".into(), "b".into(), "c".into()];
        let out = render_scrape(&state(b), Instant::now());
        assert!(out.contains("pangolin_session_routes{instance=\"work\"} 3\n"));
    }
    ...
    fn render_emits_help_and_type_lines_for_every_metric() {
        ...
        let out = render_scrape(&s, Instant::now());
        for metric in [
            "pangolin_session_info",
            "pangolin_session_state",
            "pangolin_session_started_at_unix",
            "pangolin_session_uptime_seconds",
            "pangolin_session_routes",
            "pangolin_reconnect_attempts_total",
            "pangolin_tunnel_restarts_total",
        ] {
            assert!(
    ...
    async fn http_endpoint_serves_metrics_on_localhost() {
        ...
        assert!(body.contains("pangolin_session_info{"));
