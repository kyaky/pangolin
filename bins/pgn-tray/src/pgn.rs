//! Shell out to `pgn.exe` for connect/disconnect/status.
...
/// Find pgn.exe next to pgn-tray.exe, or fall back to PATH.
pub fn pgn_exe() -> PathBuf {
    let mut path = std::env::current_exe().unwrap_or_else(|_| PathBuf::from("pgn-tray.exe"));
    path.set_file_name(if cfg!(windows) { "pgn.exe" } else { "pgn" });
    if path.exists() {
        path
    } else {
        PathBuf::from(if cfg!(windows) { "pgn.exe" } else { "pgn" })
    }
}
...
/// Launch `pgn connect` with admin elevation.
pub fn connect(profile: &str) {
    ...
    #[cfg(windows)]
    {
        ...
        if (result as isize) <= 32 {
            eprintln!(
                "pgn-tray: failed to launch pgn.exe (ShellExecuteW returned {})",
                result as isize
            );
        }
    }
    ...
    #[cfg(not(windows))]
    {
        ...
        let _ = Command::new("sudo").arg("-E").arg(&pgn).args(&args).spawn();
    }
}
